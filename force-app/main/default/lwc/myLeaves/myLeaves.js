...
const COLUMNS = [
        ...
        { label: 'Status', fieldName: 'Leave_Status__c', cellAttributes: { class: { fieldName: 'cellClass' } } },
...
export default class MyLeaves extends LightningElement {
    ...
    @api recordid;
    currentUserId = Id;
    @wire(getMyLeaves, {recordId:'$recordid'})
    wiredMyLeaves(result) {
        ...
        if (result.data) {
            this.myLeaves = result.data.map(a => ({
                ...a,
                cellClass: a.Leave_Status__c == 'Approved' ? 'slds-theme_success' : a.Leave_Status__c == 'Rejected' ? 'slds-theme_warning' : '',
                isEditDisabled: a.Leave_Status__c != 'Submitted'
            }));
        }
        if (result.error) {
